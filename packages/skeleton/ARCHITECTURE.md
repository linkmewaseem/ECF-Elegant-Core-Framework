# @ecfjs/skeleton — Package Architecture

`@ecfjs/skeleton` project scaffolding ke liye source-of-truth hai. Ye khud ek runnable package nahi hai (koi exported class/function nahi deta) — ye sirf **blueprint templates** ka collection hai jo `@ecfjs/cli` copy karke naya project banata hai.
...
- **`ecf.config.js` per blueprint** — project metadata jo `@ecfjs/cli` aur `@ecfjs/devkit` (jaise `ecf doctor`) read karte hain: project naam, blueprint type, declared packages, custom paths.
...
1. **Sirf `@ecfjs/*` package imports use hongi.** Kabhi bhi relative path se doosre package ke `src/` folder mein import mat karo (jaise `'../../../../core/src/index.js'`). Blueprint monorepo se **bahar copy hoke chalna chahiye** — relative monorepo paths isko tod dete hain.

2. **Har blueprint apne relevant Service Providers khud register kare** `bootstrap/app.js` mein. Koi bhi provider "chup-chaap kaam kar raha hai" assume mat karo — agar `@ecfjs/view` use ho raha hai to `ViewServiceProvider` explicitly register hona chahiye.
...
| `@ecfjs/http` | ✅ | ✅ |
| `@ecfjs/database` | ✅ | ✅ |
| `@ecfjs/auth` | ✅ (session guard) | ✅ (JWT guard) |
| `@ecfjs/validation` | ✅ | ✅ |
| `@ecfjs/view` | ✅ | ❌ |
...
- Skeleton ek **consumer package** hai — ise koi core infrastructure package (`@ecfjs/core`, `@ecfjs/http`, wagera) import nahi karega. Dependency direction hamesha ek-tarfa hai: skeleton → framework packages, kabhi ulta nahi.
...
4. `@ecfjs/cli`'s `ecf new --type=<name>` mein wire karo