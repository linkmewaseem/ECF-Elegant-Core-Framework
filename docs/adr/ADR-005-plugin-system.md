...
## Status
**Accepted** (Implemented in `@ecfjs/extensions`)
...
## Decision
1. Package extensions under `@ecfjs/extensions` as independent plugin modules (`@ecfjs/soft-deletes`, `@ecfjs/timestamps`, `@ecfjs/uuids`, `@ecfjs/sluggable`, `@ecfjs/audit`).
...
### Positive
- Core ORM (`@ecfjs/database`) remains lean, fast, and unbloated.
