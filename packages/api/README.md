...
- **Granular Multi-Level Rate Limiting**: Per User, Token, IP, and Route rate limiters integrated with `@ecf/cache`.
...
- **Request Correlation Tracing**: `X-Request-ID` and `X-Correlation-ID` headers linked to `@ecf/observability`.
...
```javascript
import { Api, ApiResource } from "@ecf/api";
