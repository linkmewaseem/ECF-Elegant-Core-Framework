...
## Status
**Accepted** (Implemented in `@ecf/api`)
...
3. **Advanced Rate Limiting & Security**:
   Multi-granularity rate limiting (per User, per Token, per IP, per Route) integrated with `@ecf/cache`. Supports Bearer tokens, JWT, PAT, and API Keys via `@ecf/auth`.
4. **OpenAPI 3.0 Generator & Swagger UI**:
   Automatically inspects registered `@ecf/http` routes and produces compliant `openapi.json` / `swagger.json` specs while serving interactive Swagger UI HTML dashboard at `/docs/api`.
...
6. **Idempotency, ETags & Request Correlation**:
   `IdempotencyMiddleware` prevents duplicate POST actions via `@ecf/cache`. `ETagMiddleware` calculates response hashes returning `304 Not Modified`. `CorrelationIdMiddleware` assigns `X-Request-ID` and `X-Correlation-ID`.
7. **API Profiles**:
