...
export class Pipeline {
  ...
  /**
   * Terminate all pipes that implement a terminate() method.
   * @param {...any} args
   */
  async terminate(...args) {
    const params = args.length > 0 ? args : this.passables;
    for (const pipe of this.pipes) {
      if (pipe && typeof pipe.terminate === "function") {
        await pipe.terminate(...params);
      }
    }
  }
