...
## Status
**Accepted** (Implemented in `@ecf/http`)
...
## Decision
1. Implement a Radix / Trie-based route matching engine (`TrieRouter`) inside `@ecf/http` for $O(K)$ path resolution where $K$ is path length.
